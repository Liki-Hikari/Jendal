...
import { MessageCircle, Send, User, Trash2, X } from 'lucide-react';
...
export default function BuyerMessagesPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Fetch ALL profiles at once (simpler, avoids .in() filter issues)
  ...
  const sendReply = async () => {
    ...
    const { error } = await (supabase.from('messages') as any).insert({
      ...
      product_id: activeThread.messages[0]?.product_id || null,
      subject: activeThread.messages[0]?.subject || 'Re: Inquiry',
      body: replyText.trim(),
      broadcast: false,
    });
    if (error) { toast.error('Failed to send'); }
      ...
      const { data: msgs } = await (supabase.from('messages') as any)
        .select('*')
        .or(`recipient_id.eq.${userId},sender_id.eq.${userId},broadcast.eq.true`)
        .order('created_at', { ascending: false });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <p className="text-slate-600 mt-1">Chat with sellers about your orders</p>
      </motion.div>
            ...
            <div className="space-y-1">
              {threads.map(t => (
                <div key={t.key} className="relative group">
                  <button onClick={() => setActiveThreadKey(t.key)}
                    className={`w-full text-left p-3 pr-8 rounded-lg transition-colors ${activeThreadKey === t.key ? 'bg-emerald-50 border border-emerald-200' : 'hover:bg-slate-50 border border-transparent'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <User size={14} className="text-slate-400 shrink-0" />
                      <span className="font-medium text-sm text-slate-900 truncate">{t.otherName}</span>
                    </div>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteThread(t.key); }}
          ...
          {activeThread ? (
            <>
              <div className="border-b border-slate-100 pb-3 mb-3 flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-slate-900 text-lg">{activeThread.otherName}</h3>
                  <p className="text-sm text-slate-500">{activeThread.productName}</p>
                </div>
                <button onClick={() => deleteThread(activeThread.key)}
                  ...
                  <div key={m.id} className={`flex ${m.sender_id === userId ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-lg ${m.sender_id === userId ? 'bg-emerald-50' : 'bg-slate-50'}`}>
                      <p className="text-xs font-medium text-slate-500 mb-1">
                        {getName(m.sender_id, m.broadcast)}
                      </p>
              ...
              <div className="flex gap-2">
                <input type="text" value={replyText} onChange={e => setReplyText(e.target.value)}
                  ...
                  placeholder={`Reply to ${activeThread.otherName}...`}
                ...
                <button onClick={sendReply} disabled={sending || !replyText.trim()}
                  className="px-4 py-2 bg-emerald-700 text-white rounded-lg hover:bg-emerald-800 disabled:opacity-50 transition-colors flex items-center gap-2">
                  <Send size={16} /> Send
                </button>
              </div>
            </>
